import { setGridStyles, aboutMeExpand, skillsExpand, contactExpand, layoutRespond} from './cardsLayout.js'
import { displayShowLayout, hydeShowLayout, showContent, hydeContent } from './showLayout.js'

function createRows(string, columns, container) {                                                       
  for (var i = 0; i < (string.length / columns); i++) {                       
...
function dataSetRows(i, columns){
  ...
  if (elements[i].textContent != " ") {
    elements[i].classList.add("box-shadow");
    elements[i].setAttribute("data-row", `${(Math.ceil(((i) / columns)+0.1))}`);
    elements[i].setAttribute("data-active", "true");                        
    elements[i].setAttribute("data-merged", "false")}  else {elements[i].style.width = "0px"
  };
}

function createElements(string, columns, fontWeight){                                                   
  for (var i = 0; i < string.length; i++) {
    var row = document.querySelector(`#row${(Math.ceil(((i) / columns)+0.1))}`)
...
function hyde(i) {
  var elements = document.querySelectorAll(".character-container");
  if (elements[i+1].textContent != " ") {elements[i+1].dataset.active = "false";}
  deleteWidth(i+1)
}
...
function deleteWidth(i) {
  ...
  setTimeout(() => {elements[i].classList.remove("box-shadow")}, 1000)
...
export function setLogo() {
  setTimeout(() => {
    ...
    setInterval(() => {action("gabriel ardèvol")}, 3000);
  }, 1);
}
